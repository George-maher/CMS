  ...
  useEffect(() => {
    if (show) {
      const timer = setTimeout(() => {
        document.getElementById('pwa-modal-close')?.focus()
      }, 100)
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <h3 className={`text-base font-bold truncate ${isDark ? 'text-white' : 'text-[#1a1a2e]'}`}>
                  {isIOSGuide ? t('pwa.iosInstallTitle') : t('pwa.installTitle')}
                </h3>
                <p className={`text-xs mt-0.5 truncate ${isDark ? 'text-[#94a3b8]' : 'text-[#5b5b6e]'}`}>
                  {isIOSGuide ? t('pwa.iosInstallDescription') : t('pwa.installDescription')}
                </p>
              </div>
            </div>
            <button
