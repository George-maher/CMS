...
import { getMyClassesLeaderboards, getGlobalLeaderboard } from '@/api/points'
import type { LeaderboardEntry } from '@/types'

export default function ServantLeaderboard() {
  const { t } = useTranslation()
  const [classesData, setClassesData] = useState<{ class: { id: number; name: string }; stage: { id: number; name: string } | null; leaderboard: LeaderboardEntry[] }[]>([])
  const [global, setGlobal] = useState<LeaderboardEntry[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    Promise.all([
      getMyClassesLeaderboards().catch(() => []),
  ...
  return (
    <div className="space-y-10">
      ...
      {/* ─── My Classes Leaderboards ─── */}
      {classesData.map((item, si) => (
        <MotionDiv key={item.class.id} animation="fade-in-up" delay={si * 80}>
          <section>
            <h2 className="mb-5 text-xl font-bold border-b border-gold-400/20 pb-3 gold-text flex items-center gap-2">
              <BookOpen className="h-5 w-5" />
              {item.stage ? `${item.stage.name} — ` : ''}{item.class.name}
            </h2>
