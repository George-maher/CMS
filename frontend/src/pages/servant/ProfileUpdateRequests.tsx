...
import { CheckCircle, XCircle, Clock, FileText, AlertCircle, UserCheck, ChevronRight, Eye, User, Mail, Phone, MapPin } from 'lucide-react'
        ...
        {pendingCount > 0 && (
          <div className="flex items-center gap-2 rounded-xl bg-warning-50 dark:bg-warning-900/20 border border-warning-200 dark:border-warning-800 px-3 py-2.5 shadow-sm">
            ...
            <span className="text-sm font-medium text-warning-700 dark:text-warning-300">
              {pendingCount} {pendingCount === 1 ? t('common.pending') : t('common.pending')}
            </span>
          </div>
        )}
      </div>
                ...
                <tbody>
                  {requests.map((req) => (
                    <tr key={req.id} className="group hover:bg-surface-hover/50 transition-colors">
                      <td>
                        <div className="flex items-center gap-3">
                          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full gold-gradient text-sm font-bold text-navy-900 ring-2 ring-gold-400/20 shadow-sm">
                      ...
                      <td>
                        <div className="flex flex-col">
                          <span className="text-sm font-medium">{req.user?.classe?.name || '-'}</span>
                      ...
                      <td className="text-end">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium text-primary-600 hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-colors"
                        >
          ...
          <div className="sm:hidden space-y-3">
            {requests.map((req) => (
              <button
                key={req.id}
                onClick={() => openDetail(req)}
                className="w-full card p-4 text-start hover:bg-surface-hover transition-all active:scale-[0.98]"
              >
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full gold-gradient text-sm font-bold text-navy-900 ring-2 ring-gold-400/20 shadow-sm">
                      {req.user?.name?.charAt(0).toUpperCase()}
                    </div>
                ...
                <div className="flex items-center justify-between text-xs text-muted pt-2 border-t border-border-light">
                  <span className="font-medium">{req.user?.classe?.name || '-'}</span>
            ...
            <div className="flex items-center gap-4 rounded-xl bg-surface-secondary/50 p-4 border border-border-light">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-sm font-medium">{detail.user?.classe?.name}</span>
                  {detail.user?.classe?.stage && (
                    <>
                      <span className="text-muted">·</span>
                      <span className="text-sm text-muted">{detail.user.classe.stage.name}</span>
                    </>
                  )}
                </div>
              </div>
